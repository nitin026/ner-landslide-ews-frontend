...
Adopted verbatim from the streaming service in `ml/streaming_api/stream_bus.py`,
which had solved this properly already. The platform backend previously fanned out
to every websocket client with no filtering and no backpressure policy: a map
subscribed to one district received the whole region's telemetry, and one slow
client could stall the broadcast loop for everyone. Rewriting that from scratch
would have produced a worse version of a component that already existed in the
repository.
...
# Platform singleton
# --------------------------------------------------------------------------- #
# One bus for the process, defined here rather than in `main` so the telemetry
# driver and the routers can publish without importing the app (which would be a
# circular import). This is what lets a manually stepped tick or an applied
# scenario reach a connected dashboard: previously only the background loop
# published, so anything triggered from the UI streamed nothing at all.
bus = StreamBus()

# Maps the platform's internal websocket message types onto the bus event
# catalogue, so a client filters with `events=zone_alert` rather than having to
# know our internal type strings.
PLATFORM_EVENTS = {
    "RISK_CYCLE": EVENT_ZONE_RISK,
    "ALERT": EVENT_ZONE_ALERT,
    "TELEMETRY": EVENT_SENSOR_READING,
    "SENSOR_HEALTH": EVENT_SENSOR_HEALTH,
    "SENSOR_FAULT": EVENT_SENSOR_FAULT,
}
