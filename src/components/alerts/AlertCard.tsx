...
export function AlertCard({
  ...
  return (
    <article className="alert-card" style={{ borderLeftColor: riskVar(alert.severity) }}>
      ...
      <div className="tiny">
        <strong>Roads:</strong> {alert.affectedRoads.join(", ") || "None"} · <strong>Villages:</strong>{" "}
        {alert.affectedVillages.join(", ") || "None"}
      </div>
      ...
      <div className="ac-actions noprint">
        ...
        {onAcknowledge && (
          <button
            className="btn sm"
            type="button"
            disabled={busy || (alert.status !== "NEW" && alert.status !== "ACKNOWLEDGED")}
            style={
              isAck
                ? {
            ...
            title={isAck ? "Click to reset to unacknowledged" : "Click to acknowledge"}
          >
            {isAck ? "Acknowledged (Undo)" : "Acknowledge"}
          </button>
        )}
        {onDispatch && (
          <button
