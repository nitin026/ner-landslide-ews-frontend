...
// NCS/USGS seismic triggers, Open-Meteo/IMD soil moisture, GSI Bhukosh
// susceptibility and OSM infrastructure are all served as GIS layers via
// getLayer(); these two extra endpoints back the summary strip and the
// manual "refresh live feeds" action.

export interface LiveTelemetrySummary {
  updated_at?: string;
  seismic_events_count?: number;
  nearest_seismic_km?: number;
  weather_stations_count?: number;
  avg_saturation_ratio?: number;
  gsi_zones_count?: number;
  osm_assets_count?: number;
  sources?: string[];
}

/** GET /api/gis/live/summary — Aggregated telemetry counters from open-source feeds. */
export const getLiveSummary = () => get<LiveTelemetrySummary>("/gis/live/summary");

/** POST /api/gis/live/refresh — Trigger live re-fetch across NCS, USGS, Open-Meteo, GSI, OSM. */
export const refreshLiveFeeds = () => post<LiveTelemetrySummary>("/gis/live/refresh", {});
